...
class AccessTokenReader {
    ...
    private static $prefix_accessToken = "Mob:Session:AccessToken:";//来源:\Snake\MobPackage\Session\Helper\MemcacheAccessTokenHelper;
    private $token = NULL;
    ...
    private function initTokenData() {
        ...
        $tokenData = $this->getTokenDataFromCache();
        if (!empty($tokenData)) {
    ...
    private function getTokenDataFromCache() {
        $key = self::$prefix_accessToken . $this->token;
        $res = Memcache::instance()->get($key);
        if ($res) {
            return $res;
        }
        else {
